...
AgentName = Literal["claude-code", "codex-cli", "cursor"]
...
class Orchestrator:
    ...
    def __init__(
        ...
        self.available_agents = available_agents or ["claude-code", "codex-cli"]
    ...
    def _execute_with_codex(self, assignment: Assignment) -> dict[str, Any]:
        """Execute task with Codex CLI."""
        # Determine approval mode based on permission mode
        approval_mode = "suggest"
        if assignment.permission_mode == "full":
            approval_mode = "full-auto"
        elif assignment.permission_mode == "auto":
        ...
        # Find appropriate flow for task
        flow = self._find_flow_for_task(assignment.task_id)
        if flow:
            result = adapter.execute_flow(flow, {"task": assignment.task_id})
            return {
                "success": result.success,
                "result": f"Completed {result.completed_steps}/{len(result.steps)} steps",
                "files_modified": result.files_modified,
            }
        else:
            # Simple prompt execution
            task = self.analyze_task(assignment.task_id)
            step_result = adapter.execute_step(
                step=Flow(name="task").steps[0] if Flow(name="task").steps else None,
                context={"task": assignment.task_id, "focus": task.description},
    ...
    def _find_flow_for_task(self, task_id: str) -> Optional[Flow]:
        """Find an appropriate flow for a task."""
        flows_dir = self.project_root / ".paircoder" / "flows"
        if not flows_dir.exists():
            return None

        task = self.analyze_task(task_id)

        # Map task type to flow
        flow_mapping = {
            TaskType.DESIGN: "design-plan-implement",
            TaskType.IMPLEMENT: "tdd-implement",
            TaskType.REVIEW: "code-review",
            TaskType.FIX: "tdd-implement",
        }

        flow_name = flow_mapping.get(task.task_type, "tdd-implement")
        flow_path = flows_dir / f"{flow_name}.flow.md"
