---
id: T29.8
title: Create Auto-Checkpoint on Sandbox Entry
status: pending
priority: P1
complexity: 25
...
Ensure a git checkpoint is created before entering sandbox mode, enabling easy rollback if needed.

# Files to Update

| File | Change |
|------|--------|
| `tools/cli/bpsai_pair/security/checkpoint.py` | Add `create_sandbox_checkpoint()` and `rollback_to_checkpoint()` functions |
| `tools/cli/bpsai_pair/commands/session.py` | Add `sandbox rollback` command |

# Implementation Plan

1. Create `create_sandbox_checkpoint()` function:
...
3. Add `sandbox rollback` command:
   - Find most recent sandbox checkpoint
...
- [ ] Checkpoint created with descriptive name format (`sandbox-YYYYMMDD-HHMMSS`)
...
print(create_sandbox_checkpoint())
"
...
# Test rollback
bpsai-pair sandbox rollback --dry-run
```
...
def rollback_to_checkpoint(checkpoint_id: str, force: bool = False) -> bool:
    """
    Rollback to a sandbox checkpoint.
