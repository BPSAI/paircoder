...
| `CLAUDE.md` | Add sandbox instructions |
...
Automatic on sandbox entry unless `--skip-checkpoint`.

### Rolling Back
$ bpsai-pair sandbox rollback

### Manual Recovery
$ git reset --hard sandbox-YYYYMMDD-HHMMSS
...
Exit sandbox mode, make the change manually or with normal commands.

## Troubleshooting

### "Cannot modify locked path" error
You're in sandbox mode and trying to edit a protected file. Exit sandbox
or use `sandbox rollback`.

### Network restriction errors
The domain isn't in `allow_network`. Add it to config or work offline.

### Checkpoint rollback issues
Ensure checkpoint tag exists: `git tag -l "sandbox-*"`
```
