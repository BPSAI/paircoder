---
id: T29.7
title: Test Sandbox Escape Attempts
status: pending
priority: P0
complexity: 45
...
Create comprehensive tests that attempt to bypass sandbox restrictions. These tests ensure the sandbox is secure against common bypass techniques.

# Files to Update

| File | Change |
|------|--------|
| `tests/security/test_sandbox_escapes.py` | Create comprehensive escape test suite |

# Implementation Plan

1. Create test fixtures for sandbox environment
...
pytest tests/security/test_sandbox_escapes.py -v

# Verify count
pytest tests/security/test_sandbox_escapes.py --collect-only | grep "test_" | wc -l
# Should be >= 20
```
...
from bpsai_pair.security.sandbox import SandboxManager, SandboxViolationError
...
def sandbox_manager(tmp_path):
    ...
    manager = SandboxManager(config, tmp_path)
    manager.activate()
...
class TestSymlinkEscapes:
    ...
    def test_symlink_to_locked_dir(self, sandbox_manager):
        manager, root = sandbox_manager
    ...
    def test_symlink_chain_to_locked_file(self, sandbox_manager):
        manager, root = sandbox_manager
...
class TestPathTraversalEscapes:
    ...
    def test_dotdot_traversal(self, sandbox_manager):
        manager, root = sandbox_manager
...
class TestFilesystemEscapes:
    ...
    def test_rename_locked_directory(self, sandbox_manager):
        manager, root = sandbox_manager
