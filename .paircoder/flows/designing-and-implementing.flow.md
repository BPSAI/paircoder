...
- Simple bug fixes (use `implementing-with-tdd` instead)
...
2. Run the `implementing-with-tdd` subflow:
...
Run the `finishing-branches` flow:
- Final verification
- Update state
- Prepare for merge
...
- `implementing-with-tdd` — For implementing each task
- `reviewing-code` — For code review
- `finishing-branches` — For completing the branch
