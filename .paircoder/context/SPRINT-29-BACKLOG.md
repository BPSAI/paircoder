...
- [ ] Sandbox status visible in `bpsai-pair status` output
...
sandbox:
  ...
  # Rollback on sandbox violation attempts
...
#### T29.1: Design Sandbox Config Schema
**Priority:** P0  
**Complexity:** 25  
**Effort:** 2 hours  
**Dependencies:** None

**Objective:** Define the complete sandbox configuration schema that will control contained autonomy behavior.

**Acceptance Criteria:**
- [ ] Schema defined in Pydantic model (SandboxConfig)
...
#### T29.2: Add Sandbox Section to config.yaml
**Priority:** P0  
**Complexity:** 20  
**Effort:** 2 hours  
**Dependencies:** T29.1

**Objective:** Wire the sandbox config schema into the main config loading and validation system.
...
**Files to Modify:**
- `tools/cli/bpsai_pair/core/config.py` - Add SandboxConfig to ConfigModel
...
### Phase 2: Core Sandbox Implementation (T29.3-T29.5)

#### T29.3: Implement Directory Locking in sandbox.py
**Priority:** P0  
**Complexity:** 45  
**Effort:** 4 hours  
**Dependencies:** T29.2
...
def contained_auto(
    ...
    if config.sandbox.auto_checkpoint and not skip_checkpoint:
        checkpoint_id = create_checkpoint("sandbox-entry")
...
**Objective:** Restrict network access to only allowed domains during contained mode.
    ...
    def check_url(self, url: str) -> None:
        ...
        if not self._is_allowed(domain):
            raise NetworkRestrictionError(
                f"Network access to '{domain}' blocked in sandbox mode.\n"
...
#### T29.7: Test Sandbox Escape Attempts
**Priority:** P0  
**Complexity:** 45  
**Effort:** 4 hours  
**Dependencies:** T29.4

**Objective:** Create comprehensive tests that attempt to bypass sandbox restrictions.
...
#### T29.8: Create Auto-Checkpoint on Sandbox Entry
**Priority:** P1  
**Complexity:** 25  
**Effort:** 2 hours  
**Dependencies:** T29.4

**Objective:** Ensure a git checkpoint is created before entering sandbox mode, enabling easy rollback.

**Acceptance Criteria:**
- [ ] Checkpoint created with descriptive name
- [ ] Checkpoint ID stored in environment
- [ ] `bpsai-pair sandbox rollback` restores checkpoint
...
def create_sandbox_checkpoint() -> str:
    """
...
#### T29.9: Add Sandbox Status to `bpsai-pair status`
**Priority:** P1  
**Complexity:** 20  
**Effort:** 2 hours  
**Dependencies:** T29.4

**Objective:** Show sandbox status in the main status command output.

**Acceptance Criteria:**
- [ ] Sandbox section appears in status output when sandbox enabled
...
- Status hides sandbox section when disabled
...
**Objective:** Document how to properly invoke subagents (planner, reviewer, security) in contained mode.
...
- [ ] Integration with sandbox mode documented
...
Subagents are read-only in contained mode.
```
...
|------|--------|------------|------------|
| Sandbox too restrictive for normal work | HIGH | MEDIUM | Make contained mode opt-in, not default |
